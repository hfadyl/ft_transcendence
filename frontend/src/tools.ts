...
export const roomNameValidation = (name: string) => {
  ...
  if (name.length > 20) return "Name must be at most 20 characters long";
  if (!/^[\s\w]{3,20}$/.test(name)) return "Name must only contain letters and numbers";
  return "";
};

export const getLevel = (score: number) => {
  if (score < 100) return "Newbie";
  if (score < 200) return "Nadi";
  if (score < 300) return "Agent";
  if (score < 400) return "Expic";
  if (score < 500) return "Legend";
  if (score >= 500) return "3ankoob";
  return "Newbie";
};

export const getLevelProgress = (score: number) => {
  if (score < 100) return score;
  if (score < 200) return score - 100;
  if (score < 300) return score - 200;
  if (score < 400) return score - 300;
  if (score < 500) return score - 400;
